/* MapReduce Program - file XmlInputFormat.java
 * Author:
 * Thomas P. Moyer
 */
...
public class XmlInputFormat extends TextInputFormat {
    ...
    public RecordReader<LongWritable,
            Text> createRecordReader(InputSplit is, TaskAttemptContext tac) {
    ...
    public static class XmlRecordReader extends RecordReader<LongWritable, Text> {
                ...
                if (readUntilMatch(startTag, false)) {
                    ...
                    try {
                        ...
                        if (readUntilMatch(endTag, true)) {
                            ...
                            logger.debug("false= readUntilMatch but buffer not 0length.  "
                                    + "This will show only for xmlinput.start "
                                    + "with no xmlinput.end tags");
                            value.set(buffer.getData(), 0, buffer.getLength());
        ...
        private boolean readUntilMatch(byte[] match, boolean withinBlock) throws IOException {
            ...
            while (true) {
                 ...
                 * It continues
                 *  reading past the file-split in log file
                 * !!! yourjobIdBelowHere         !!!
                 *  http://localhost.localdomain:50060/tasklog?attemptid=
                 * attempt_201310220813_0030_m_000000_0&all=true
                 *  The second behavior is when readUntilMatch is called
                 * with match==the xmlinput.start
                 *  at the beginning of the map-00001 (second input split).
                 *  http://localhost.localdomain:50060/tasklog?attemptid=
                 * attempt_201310220813_0030_m_000001_0&all=true
                 *  By reading up to the xmlinput.start it ignores
                 * the partial page "overRead" by mapper-just-prior
                 */
                //if(  (fsin.getPos()>=67108864) /* the beginning of split the second */
                //&&(fsin.getPos()<=67158079) /* for the version of wikipedia I grabbed,
                // bendodiazapine
                //started at byte 67042129 and ended at 67157331
                // (spanning the 0th and 1st input splits)*/
                //){
                ///**/logger.debug(String.format("%10d byte=%3d %c",fsin.getPos(),b,(char)b));
                ///* mapper log gets 1 line per inputfile byte */
                //}

                if (b == -1) {
                    return false; /* end of file */
                }
                denyLeadingSpaces = ((denyLeadingSpaces && (32 == b))
                        || (10 == b) || (13 == b)) ? true : false;
                // save to buffer:
                if (withinBlock) {
                    if ((b != 10)
                            && (b != 13)
